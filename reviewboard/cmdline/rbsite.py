...
class InstallCommand(Command):
    ...
    def chown_files(self):
        """
        Change the ownership of the files that need to belong to the
        web server.
        """
        directories = [
            os.path.join(site.abs_install_dir, 'htdocs', 'media', 'uploaded'),
        ...
        for path in directories:
            for root, dirs, files in os.walk(path):
                ...
                # Set the ownership of all child paths.
                for child_path in itertools.chain(dirs, files):
                    joined_path = os.path.join(root, child_path)

                    try:
                        os.chown(joined_path, uid, gid)
                    except OSError:
                        ui.error("Could not set ownership of %s" % joined_path)
                        site.display_chown_message = True
